...
`raw_premise` 同时作为后续封面唯一标题源，因此必须是简短、直接、准确的反事实主题句，默认简体中文，直接点明核心变化，不使用悬念标题、夸张点击诱导或第二种改写。例如：`如果月球永久消失`。`raw_premise` 只承担主题标题身份；更完整的作用域、持续性和限制写入 `premise`。后续任何阶段不得改写 `raw_premise` 作为封面标题。
