...
def main() -> None:
    ...
    history = [
        ...
        {"date": "2026-08-12", "scope": "太空", "subscope": "celestial_body", "anchor": "月球"},
    ]
    candidates = {
        "candidates": [
    ...
    def eval_call(_: str) -> dict:
        ...
        for anchor, values in scores.items():
            items.append({
                "anchor": anchor,
                "curiosity": values[0],
                "recognizability": values[1],
                "consequence_depth": values[2],
                "visual_potential": values[3],
                "non_obviousness": values[4],
                "propagation_viability": values[5],
                "scope_match": values[6],
                "duplicate_or_rephrase": values[7],
            })
    ...
    gate = select_daily_anchor(
        ...
        history=history,
        candidate_model_call=candidate_call,
        evaluation_model_call=eval_call,
    )
