...
function buildComposition({ renderPlan, projectRoot, outputDir, fontPath }) {
  ...
  const visualHtml = renderPlan.layers.map((layer, index) => {
    ...
    if (layer.treatment === 'direct_video') {
      return `<video id="${id}" class="hf-visual" src="${esc(src)}" data-start="${start}" data-duration="${duration}" data-track-index="1" muted playsinline preload="auto"></video>`;
    }
    if (layer.treatment === 'static_image') {
      return `<img id="${id}" class="hf-visual" src="${esc(src)}" data-start="${start}" data-duration="${duration}" data-track-index="1" />`;
    }
    if (layer.treatment === 'image_motion') {
      const resolved = normalizeMotion(layer.motion);
      motionRecords.push({ id, start, duration, ...resolved });
      return `<img id="${id}" class="hf-visual hf-motion" src="${esc(src)}" data-start="${start}" data-duration="${duration}" data-track-index="1" />`;
    }
...
#root { position:relative; width:1280px; height:720px; overflow:hidden; background:#000; }
.hf-visual { position:absolute; inset:0; width:1280px; height:720px; object-fit:cover; transform-origin:center center; }
.hf-subtitle { position:absolute; left:80px; right:80px; bottom:52px; z-index:100; text-align:center; color:white; font-family:'Noto Sans CJK SC',sans-serif; font-size:38px; line-height:1.35; font-weight:400; text-shadow:0 2px 8px rgba(0,0,0,.85); }
</style>
</head>
<body>
<div id="root" data-composition-id="main" data-start="0" data-width="1280" data-height="720" data-duration="${asSeconds(durationMs)}">
${visualHtml}
${audioHtml}
${subtitleHtml}
</div>
...
if (require.main === module) {
  ...
  const result = buildComposition({ renderPlan, projectRoot, outputDir, fontPath: 'video-production/assets/fonts/NotoSansCJKsc-Regular.otf' });
